...
<body data-theme="light">
    ...
    @endphp
    @if($sliders->count())
    <div class="slider-container" style="max-width:900px;margin:24px auto 0;position:relative;overflow:hidden;border-radius:1rem;box-shadow:0 2px 12px #0001;">
        <div id="sliderImages" class="slider-images" style="display:flex;transition:transform 0.5s;">
            @foreach(is_iterable($sliders) ? $sliders : [] as $slider)
                <a href="{{ $slider->link ?: '#' }}" target="_blank" style="flex:0 0 100%;display:block;">
                    <img src="{{ asset('storage/'.$slider->image) }}" alt="{{ $slider->title }}" style="width:100%;height:220px;object-fit:cover;display:block;">
                </a>
            @endforeach
        </div>
        <button id="sliderPrev" class="slider-arrow slider-arrow-right" style="position:absolute;top:50%;right:10px;transform:translateY(-50%);background:#fff8;border:none;border-radius:50%;width:40px;height:40px;display:flex;align-items:center;justify-content:center;box-shadow:0 2px 8px #0002;cursor:pointer;z-index:2;">
        ...
        <button id="sliderNext" class="slider-arrow slider-arrow-left" style="position:absolute;top:50%;left:10px;transform:translateY(-50%);background:#fff8;border:none;border-radius:50%;width:40px;height:40px;display:flex;align-items:center;justify-content:center;box-shadow:0 2px 8px #0002;cursor:pointer;z-index:2;">
        ...
        <div class="slider-dots" style="position:absolute;bottom:12px;left:0;right:0;display:flex;justify-content:center;gap:8px;z-index:2;">
            @foreach(is_iterable($sliders) ? $sliders : [] as $i=>$slider)
                <span class="slider-dot" data-index="{{ $i }}" style="width:10px;height:10px;border-radius:50%;background:#fff8;border:2px solid #0ea5e9;display:inline-block;cursor:pointer;"></span>
            @endforeach
        </div>
    </div>
    <script>
        ...
        let sliderIndex = 0;
        const sliderCount = {{ $sliders->count() }};
        function updateSlider() {
            sliderImages.style.transform = `translateX(-${sliderIndex * 100}%)`;
            sliderDots.forEach((dot,i)=>{
                dot.style.background = i===sliderIndex ? '#0ea5e9' : '#fff8';
            });
        }
        prevBtn.onclick = ()=>{
    ...
    <div class="user-info-row">
        <div class="user-info-card">
            <div style="font-size:1.1rem;color:var(--primary-600);font-weight:600;margin-bottom:0.3rem;">الاسم</div>
            <div style="font-size:1.25rem;font-weight:700;color:var(--text-primary);">محمد أحمد</div>
        </div>
        <div class="user-info-card">
            <div style="font-size:1.1rem;color:var(--primary-600);font-weight:600;margin-bottom:0.3rem;">رصيد الحساب</div>
            <div style="font-size:1.25rem;font-weight:700;color:var(--text-primary);">2500 ج.م</div>
        </div>
        <div class="user-info-card">
            <div style="font-size:1.1rem;color:var(--primary-600);font-weight:600;margin-bottom:0.3rem;">رقم العضوية</div>
            <div style="font-size:1.25rem;font-weight:700;color:var(--text-primary);">#10234</div>
        </div>
        <div class="user-info-card">
            <div style="font-size:1.1rem;color:var(--primary-600);font-weight:600;margin-bottom:0.3rem;">السوق الحالي</div>
            <div style="font-size:1.25rem;font-weight:700;color:var(--text-primary);">القاهرة</div>
        </div>
    </div>
